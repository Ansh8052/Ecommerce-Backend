...
const findAllShipping = async (req,res) => {
  try {
    ...
    if (options.select && options.select.length){
      options.select = [ 'createdAt', 'updatedAt', 'addedBy', 'name', 'parentCategoryId' ].filter(Set.prototype.has, new Set(options.select));
    } else {
...
const getShipping = async (req,res) => {
  try {
    ...
    let options = {};
    let selectedFields =  [ 'createdAt', 'updatedAt', 'addedBy', 'name', 'parentCategoryId' ];
